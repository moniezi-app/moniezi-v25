...
export async function generateTaxSummaryPdfBytes(data: TaxSummaryPdfData): Promise<Uint8Array> {
  ...
  const bodyFont = await pdfDoc.embedFont(reportRegularOtf, { subset: true });
  const boldFont = await pdfDoc.embedFont(reportBoldOtf, { subset: true });
  const kickerFont = await pdfDoc.embedFont(appRegularOtf, { subset: true });
